...
Pokémon Roulette is a browser-based Angular game where players build a team through a series of roulette spins simulating a full Pokémon adventure. Types are mechanically meaningful in gym and Elite Four battles — type advantage adjusts wheel odds and is now displayed via an inline matchup strip between the battle title and wheel.

**Shipped in v1.1:**
- Inline matchup strip in both gym and Elite Four battle components
- Column layout: leader type icon(s) → advantage label → relevant team type icons
- Live-updates on PC team swap (no new subscription — driven by existing flow)
- `TypeMatchupService.getMatchupTypes()` returns typed advantage/disadvantage arrays
- Removed `typeAdvantageModal` and orphaned i18n keys — double-modal bug resolved
...
Replace the type advantage modal with a persistent inline strip between the battle title and wheel. The strip shows the leader's type icon, advantage label, and relevant team type icons — fixing the double-modal bug on multi-slot leaders.

## Requirements

- REF-01/02: Remove typeAdvantageModal from both battle components ✅
- SVC-04: TypeMatchupService.getMatchupTypes() ✅
- I18N-01: Remove strong/weak orphaned keys from 6 locales ✅
- STRIP-01–06: Inline strip in gym + E4, all states, live update ✅
...
|----------|-----------|---------|
| `type-matchups.json` at project root | User requirement; becomes a typed TS reference via `PokemonType` | ✅ Shipped |
| Any SE match counts as "strong" | User confirmed — simple and clear | ✅ Shipped |
| Recalculate on team change immediately | User wants live feedback during battle | ✅ Shipped |
| Skip Champion/Rival | User explicitly excluded for now | ✅ Deferred |
