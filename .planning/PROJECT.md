...
- ✓ Fix Pokédex modal fullscreen on mobile (MOB-01) — v1.2
- ✓ Persist shiny flag in PokedexData (SHINY-01) — v1.2
- ✓ CSS glow-pulse animation for won Pokémon cells (VIS-01) — v1.2
- ✓ Pokémon detail modal — official-artwork circle, number, name (DETAIL-01) — v1.2
- ✓ Detail modal — alternate forms (DETAIL-02) — v1.2
- ✓ Shiny toggle button in detail modal (SHINY-02) — v1.2

- ✓ Shiny flag correctly persisted after shiny roulette resolves (SHINY-03) — v1.3
- ✓ Alt-form champion wins register base national dex entry (ALTW-01) — v1.3
...
- **Shipped:** v1.2 Pokédex Visual Enhancement (2026-04-08) — 7 phases total, 11 plans, 165 passing tests
- **Architecture**: Angular 21 standalone components, RxJS BehaviorSubject services, no backend
- **Key files:** `PokedexService`, `PokedexEntryComponent`, `PokedexDetailModalComponent`, `PokedexComponent`, `pokedex-by-generation.ts`
- **localStorage key:** `'pokemon-roulette-pokedex'` — schema `{ caught: Record<string, { won: boolean, sprite: string|null, shiny?: boolean }> }`
- **Sprite source:** `front_default` from PokeAPI (grid) + official-artwork (detail modal) — URLs are deterministic and cached
...
| Component-based detail modal (not template-based) | Modal state (shiny toggle, form selection) lives inside component; no @ViewChild pollution | ✓ Good — v1.2 |
| `displayUrl` getter wrapping `artworkUrl` with hasError fallback | Clean separation between computed URL and error recovery; no template conditionals needed | ✓ Good — v1.2 |
| `getFormIds(id).length > 1` instead of `hasForms(pokemon)` | Only pokemonId available in detail modal; avoids full PokemonItem dependency | ✓ Good — v1.2 |
| Shiny flag cumulative in markSeen | Once seen as shiny, always marked — matches real game behavior | ✓ Good — v1.2 |
| Global CSS for modal-fullscreen-sm-down fix | ng-bootstrap portal renders outside component scope; component CSS cannot reach modal DOM | ✓ Good — v1.2 |

## Evolution

This document evolves at phase transitions and milestone boundaries.

## Current Milestone: v1.3 Pokédex Data Integrity

**Goal:** Fix two data-loss bugs where shiny status and alt-form champion wins were silently dropped.

**Status:** ✅ Complete — shipped 2026-04-09

### Active

_(none — all v1.3 requirements validated)_

---
*Last updated: 2026-04-09 — v1.3 started*
