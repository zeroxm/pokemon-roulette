# Phase 9 PLAN-01: Create BaseBattleRouletteComponent

## Goal

Create the abstract base class `BaseBattleRouletteComponent` that owns all duplicated
subscription lifecycle, shared fields, and shared methods currently repeated verbatim
across `gym`, `elite-four`, `champion`, and `rival` battle roulette components.

This plan is additive only — no existing file is modified. The new file must compile
cleanly and TypeScript strict mode must remain green before any component is changed.
...
No other files touched in this plan.

---

## Steps

### Step 1 — Create the directory and write the base class file

Create the file at the exact path above with the following complete content.
Do not add a `@Component` decorator — this is a plain abstract TypeScript class.
...
Run the Angular compiler in no-emit mode to confirm the new file is valid before any
...
Expected: zero errors. The base class file has no dependents yet so no existing spec
can break.
...
```bash
# TypeScript strict compile — must be zero errors
npx tsc --project tsconfig.app.json --noEmit

# Confirm file exists at expected path
Test-Path "src/app/main-game/roulette-container/roulettes/base-battle-roulette/base-battle-roulette.component.ts"
```

Both commands must succeed before proceeding to PLAN-02.
...
```
refactor(battle): add BaseBattleRouletteComponent abstract base class

Extracts shared subscription lifecycle, plusModifiers, hasPotions, and
usePotion into an abstract base. No existing component is changed yet.
Part of BATTLE-01 debt elimination.
```
